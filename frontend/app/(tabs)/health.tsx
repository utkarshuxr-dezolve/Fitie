...
export default function HealthScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    catch (e) { console.log('Health load:', e); }
  ...
  const statusClr = (s: string) => s === 'normal' ? colors.success : s === 'low' || s === 'high' ? colors.warning : colors.error;
                ...
                {selected.insights.key_metrics?.length > 0 && (
                  ...
                  {selected.insights.key_metrics.map((m: any, i: number) => (
                    <View key={i} style={s.metricRow}>
                      ...
                      <View style={[s.statusBadge, { backgroundColor: statusClr(m.status) + '1A' }]}><Text style={[s.statusText, { color: statusClr(m.status) }]}>{m.status}</Text></View>
                    </View>
                  ))}</>
                )}
...
const s = StyleSheet.create({
  ...
  reportName: { fontSize: 14, fontWeight: '600', color: colors.textMain },
  ...
  rtName: { fontSize: 15, fontWeight: '600', color: colors.textMain },
  ...
  detailScroll: { flex: 1 },
  detailContent: { paddingHorizontal: spacing.screen, paddingBottom: 32 },
  summaryBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, backgroundColor: '#F0FDF4', borderRadius: radius.lg, padding: 16, marginBottom: spacing.lg },
  summaryText: { fontSize: 14, color: colors.textMain, flex: 1, lineHeight: 20 },
  ...
  metricVal: { fontSize: 16, fontWeight: '700', color: colors.textMain },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: radius.full },
  statusText: { fontSize: 11, fontWeight: '700', textTransform: 'capitalize' },
  riskBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, backgroundColor: '#FFFBEB', borderRadius: radius.lg, padding: 14, marginBottom: 8 },
  riskText: { fontSize: 14, color: colors.textMain },
  ...
  recText: { fontSize: 14, color: colors.textMain, flex: 1, lineHeight: 20 },
});
