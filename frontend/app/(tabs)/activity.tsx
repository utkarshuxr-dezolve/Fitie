...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, RefreshControl, ActivityIndicator, Modal, Alert, TextInput } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Dumbbell, Scan, ChevronRight, Play, CheckCircle, X, Clock, Filter, Search, ArrowLeft } from 'lucide-react-native';
...
export default function ActivityScreen() {
  ...
  useEffect(() => {
    let iv: any;
    if (activeWorkout && showWorkoutModal) iv = setInterval(() => setWorkoutTimer(t => t + 1), 1000);
  ...
  const startWorkout = async (ids?: string[], planName?: string) => {
    ...
    try {
      ...
      setActiveWorkout(data); setWorkoutTimer(0); setShowWorkoutModal(true); setSelectedExercises([]);
  ...
  const completeWorkout = async () => {
    ...
    try {
      ...
      setShowWorkoutModal(false); setActiveWorkout(null); setWorkoutTimer(0);
  ...
  return (
    <SafeAreaView style={st.safe} edges={['top']}>
      <ScrollView style={st.scroll} contentContainerStyle={st.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(); }} tintColor={colors.primary} />} showsVerticalScrollIndicator={false}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={st.plansScroll}>
          {plans.map((plan, i) => (
            <TouchableOpacity key={i} testID={`plan-${plan.id}`} style={st.planCard} onPress={() => startWorkout(plan.exercises, plan.name)} activeOpacity={0.9}>
              ...
              <Text style={st.planMeta}>{plan.days_per_week}x/week · {plan.duration_weeks}wk</Text>
        ...
        {filtered.map(ex => (
          <TouchableOpacity key={ex.id} testID={`exercise-${ex.id}`} style={[st.exCard, selectedExercises.includes(ex.id) && st.exCardSel]} onPress={() => toggleEx(ex.id)} activeOpacity={0.85}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={st.exMeta}>{ex.muscle_group} · {ex.equipment} · {ex.difficulty}</Text>
            </View>
      ...
      <Modal visible={showWorkoutModal} animationType="slide">
        <SafeAreaView style={st.workoutSafe}>
          ...
          <View style={st.timerWrap}>
            <View style={st.timerCircle}><Text style={st.timerText}>{fmt(workoutTimer)}</Text><Text style={st.timerLabel}>elapsed</Text></View>
          </View>
...
const st = StyleSheet.create({
  ...
  scanTitle: { fontSize: 15, fontWeight: '700', color: colors.textMain },
  ...
  planBadgeText: { fontSize: 10, fontWeight: '700', color: '#fff', textTransform: 'uppercase' },
  planName: { fontSize: 15, fontWeight: '700', color: '#fff', marginBottom: 4 },
  ...
  exName: { fontSize: 14, fontWeight: '600', color: colors.textMain },
  ...
  detectedExTitle: { fontSize: 14, fontWeight: '600', color: colors.textMain, marginBottom: 8 },
  ...
  timerText: { fontSize: 40, fontWeight: '800', color: colors.textMain, letterSpacing: -1 },
  ...
  workoutExName: { fontSize: 14, fontWeight: '600', color: colors.textMain },
